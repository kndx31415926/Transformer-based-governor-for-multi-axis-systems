function [out, fig] = spinn3d_demo_ctcgov_online_nonn_nolimit(cfg)
% CTC + Governor（仅物理限制：tau / qd / qdd；无任何功率限制，在线仿真）
% - α：仍由 controller 的 feasible-α（主要由 qd/qdd 限制 + α 调节器）决定，用于 time-scaling；
% - 功率：只记录/绘图（BUS 口径 + Ppos），不用于缩放或限幅；
% - 用途：作为“CTC-only 但遵守速度/加速度/扭矩等物理约束”的基线（等价于 nonn 将功率护栏完全关闭）；
...
if nargin < 1 || isempty(cfg)
    ...
    assert(~isempty(blk), '[nonn-nolimit] 未提供 cfg，且未找到 spinn3d_params_block。');
...
req(gains,  {'Kp','Kd'});
% 功率上限（用于画参考线）；本脚本不做功率护栏
if isfield(limits,'P_total_max') && ~isempty(limits.P_total_max)
    assert(isscalar(limits.P_total_max) && isnumeric(limits.P_total_max), 'P_total_max 必须为数值标量');
else
    limits.P_total_max = cfg.P_TOTAL_MAX;
end
Pmax_plot = limits.P_total_max;  % 仅用于绘图/对比（红虚线）

% ---- 关闭功率限制：对 controller 与执行器都不施加功率护栏 ----
limits.P_total_max = inf;
limits.P_axis_max  = inf(size(limits.P_axis_max));
...
% 默认口径（缺省时）：about='com'（惯量关于 COM），mode='replace'
if isstruct(pay_cfg)
...
% （可选）去掉 α 调节器的额外“算法约束”（不是机械臂硬物理限制）
% 若想让 α 更接近“只由 qd/qdd 可行性决定”，将 RELAX_ALPHA_REG 设为 true。
RELAX_ALPHA_REG = false;
if RELAX_ALPHA_REG
    areg.BETA     = 1.0;  % 不做低通滤波
    areg.A_DOT_UP = inf;  % 不限 α 上升速度
    areg.A_DOT_DN = inf;  % 不限 α 下降速度
end
...
bus = get_bus_from_cfg(cfg, Pmax_plot);
...
fig = figure('Name','CTC+Gov (online nonn; NO power guard)','Visible','on');
...
title(ax3,'EE Trajectory + Robot Skeleton (Initial/Final; nonn-noPower)'); legend(ax3,'Location','best');
...
if isfinite(Pmax_plot) && Pmax_plot <= ymax, yline(axD, Pmax_plot, '--r','P_{max}'); end
xlabel(axD,'t [s]'); ylabel(axD,'Power [W]'); title(axD,'Bus Power (grid & dump)'); legend(axD,'show','Location','best');

drawnow;

%% -------- 输出 --------
out = struct('log',log,'limits',limits,'gains',gains,'cfg',cfg,'t_reach',t_reach,'Pmax_plot',Pmax_plot);
end
...
function tau = clamp_power_axis_total_bus(tau, vel, P_axis_max, bus)
    for j=1:numel(tau)
        Pj = tau(j) * vel(j);
        if Pj > P_axis_max(j)
...
function bus = get_bus_from_cfg(cfg, P_total_max)
